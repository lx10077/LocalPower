...
if not from_data:
    ...
    result = numpy.load(f'p_{method}.npy', allow_pickle=True).tolist()
...
for row in range(5):
    for col in range(3):
        ...
        if not from_data:
            ...
            _, error2 = lpm(data, k, t, 2, q=q0, vk=vk, is_decay=False)
            _, error4 = lpm(data, k, t, 4, q=q0, vk=vk, is_decay=False)
            _, error8 = lpm(data, k, t, 8, q=q0, vk=vk, is_decay=False)
            _, error16 = lpm(data, k, t, 16, q=q0, vk=vk, is_decay=False)

            result[filename] = [
                error1, error2, error4, error8, error16
            ]
            numpy.save(f'p_{method}', result)

        else:
            error1, error2, error4, error8, error16 = result[filename]

        iters = numpy.arange(t)
        ax_i.semilogy(iters, error1, '-xg', label='p=1', linewidth=1)
        ax_i.semilogy(iters, error2, '--*b', label='p=2', linewidth=1)
        ax_i.semilogy(iters, error4, '-*c', label='p=4', linewidth=1)
        ax_i.semilogy(iters, error8, '-or', label='p=8', linewidth=1)
        ax_i.semilogy(iters, error16, '-.m', label='p=16', linewidth=1)
